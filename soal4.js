...
function hitungTotalPenjualan(dataPenjualan) {
  ...
  if (!Array.isArray(dataPenjualan) || !dataPenjualan.every(item => typeof item === 'object' && 'totalTerjual' in item)) {
  }

  // Menghitung total penjualan
  const totalPenjualan = dataPenjualan.reduce((total, item) => {
      return total + (item.totalTerjual || 0);
  }, 0);
